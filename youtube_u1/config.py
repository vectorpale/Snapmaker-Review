...
**重要要求：所有结论必须有数据支撑。** 不要使用"普遍"、"公认"、"高频"等模糊描述，而是给出具体数字。例如：
- "共 {comment_count} 条评论中，约 X 条（Y%）提到了…"
- "N 位用户明确表示…（用户A：'...'，用户B：'...'）"
- 每个主题/观点，都要标注：提及该主题的评论条数、总点赞数、代表性用户名

## 1. 情感概况 (Sentiment Overview)
统计各情感倾向的评论数量和占比（正面 X 条 Y%，负面 X 条 Y%，中性 X 条 Y%，混合 X 条 Y%）。

## 2. 正面反馈主题 (Positive Themes)
逐个主题列出，每个主题包含：
- 主题名称
- 提及该主题的评论数量和这些评论的累计点赞数
...
用户提出的具体改进建议，标注提出该建议的用户数量。

## 5. 常见疑问 (Common Questions)
用户频繁提出的问题，标注提问次数。

## 6. 关键洞察 (Key Insights)
从评论数据中发现的重要信号，每条洞察都附上支撑数据（条数、点赞数等）。
"""
...
**重要要求：所有结论必须有数据支撑。** 当你说某个观点是"公认的"、"高频的"、"普遍的"时，必须给出具体数字证明。
例如：
- "X/{video_count} 个视频（Y%）提到了…（频道：A, B, C）"
- "在评论分析中，共 N 个视频的评论区出现了该话题"
- 不能出现没有数字支撑的"普遍认为"、"大多数评测者表示"等表述

请用中文撰写一份结构完整的综合分析报告（关键术语和原始表述在括号内保留英文），包括:

## 1. 执行摘要 (Executive Summary)
200字以内的核心发现概述。包含关键统计数字。
...
基于缺点统计，按优先级排序的改进建议。每条建议标注数据来源（哪些频道/多少评论提出）。

## 6. 竞品定位分析 (Competitive Positioning)
相对于Bambu Lab、Prusa等竞品的市场定位分析。引用各频道的具体对比结论。

## 7. 评测者观点一致性与分歧 (Consensus vs Disagreements)
- 一致观点：X/{video_count} 个视频持相同看法的议题，列出持该看法的频道名
- 分歧观点：不同评测者看法相反的议题，分别列出正反双方的频道名及其论据
